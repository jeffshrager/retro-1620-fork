...
Cards: text pieces = 'Vn 81 +POND' / 'Vn 81 -ERED' (+ starts a word, - continues it); counts = '<addr> 01 <n>'; '****' separates
...
for line in open(sys.argv[1]):
    ...
    if m:
        t = m.group(1)
        if t.startswith("-") and toks and toks[-1][0] == "t": toks[-1] = ("t", toks[-1][1] + t[1:])   # continuation piece
        else: toks.append(("t", t[1:] if t[:1] == "+" else t))
        continue
